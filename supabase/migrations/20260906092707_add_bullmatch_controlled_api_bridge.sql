-- BMI-APP-003: service-only database bridge for the BullMatch Edge API.
  ...
  case upper(coalesce(p_resource, ''))
      ...
      from (
        select m.id as match_id,m.match_date,m.published_at,
          jsonb_build_object('id',m.id,'match_date',m.match_date,'date_precision',m.date_precision,'match_number',m.match_number,'status',m.status,
            ...
            'participants',coalesce((select jsonb_agg(jsonb_build_object('id',mp.id,'bull_id',mp.bull_id,'side',mp.side,'display_name',mp.display_name_snapshot,
              'weight_kg',mp.weight_kg,'age_months_estimate',mp.age_months_estimate,'result',mp.participant_result) order by mp.side,mp.id)
              from bullmatch.match_participants mp where mp.match_id=m.id),'[]'::jsonb)) as item
    ...
    when 'MATCH' then
      ...
      select jsonb_build_object('id',m.id,'match_date',m.match_date,'date_precision',m.date_precision,'match_number',m.match_number,'status',m.status,
        ...
        'participants',coalesce((select jsonb_agg(jsonb_build_object('id',mp.id,'bull_id',mp.bull_id,'side',mp.side,'display_name',mp.display_name_snapshot,
          'camp_id_snapshot',mp.camp_id_snapshot,'owner_id_snapshot',mp.owner_id_snapshot,'weight_kg',mp.weight_kg,'age_months_estimate',mp.age_months_estimate,'result',mp.participant_result)
          order by mp.side,mp.id) from bullmatch.match_participants mp where mp.match_id=m.id),'[]'::jsonb)) into v_result
      from bullmatch.matches m join bullmatch.match_results mr on mr.match_id=m.id and mr.verified_at is not null and mr.result_type<>'UNKNOWN'
      left join bullmatch.events e on e.id=m.event_id left join bullmatch.venues v on v.id=m.venue_id
...
create or replace function public.bullmatch_api_member(p_user_id uuid)
returns jsonb language sql security definer set search_path=''
as $$
  select coalesce((select jsonb_build_object('member',true,'user_id',au.user_id,'display_name',au.display_name,'role',au.role,'status',au.status,'active',au.status='ACTIVE')
    from bullmatch.app_users au where au.user_id=p_user_id),jsonb_build_object('member',false,'user_id',p_user_id,'display_name',null,'role',null,'status',null,'active',false))
$$;

create or replace function public.bullmatch_api_admin_command(p_actor_id uuid,p_operation text,p_payload jsonb default '{}'::jsonb)
returns jsonb language plpgsql security definer set search_path=''
as $$
declare v_id uuid; v_op text:=lower(btrim(coalesce(p_operation,''))); v_payload jsonb:=coalesce(p_payload,'{}'::jsonb);
begin
  ...
  case v_op
    when 'create_owner' then v_id:=bullmatch.admin_create_owner(v_payload);
    when 'update_owner' then v_id:=bullmatch.admin_update_owner((v_payload->>'id')::uuid,coalesce(v_payload->'patch','{}'::jsonb));
    when 'create_camp' then v_id:=bullmatch.admin_create_camp(v_payload);
    when 'update_camp' then v_id:=bullmatch.admin_update_camp((v_payload->>'id')::uuid,coalesce(v_payload->'patch','{}'::jsonb));
    when 'create_bull' then v_id:=bullmatch.admin_create_bull(v_payload);
    when 'update_bull' then v_id:=bullmatch.admin_update_bull((v_payload->>'id')::uuid,coalesce(v_payload->'patch','{}'::jsonb));
    when 'create_venue' then v_id:=bullmatch.admin_create_venue(v_payload);
    when 'update_venue' then v_id:=bullmatch.admin_update_venue((v_payload->>'id')::uuid,coalesce(v_payload->'patch','{}'::jsonb));
    when 'archive_entity' then v_id:=bullmatch.admin_archive_entity(v_payload->>'entity_type',(v_payload->>'id')::uuid,v_payload->>'reason');
    when 'set_entity_verification' then v_id:=bullmatch.admin_set_entity_verification(v_payload->>'entity_type',(v_payload->>'id')::uuid,v_payload->>'status');
    when 'upsert_alias' then v_id:=bullmatch.admin_upsert_alias(v_payload->>'entity_type',(v_payload->>'id')::uuid,v_payload->>'alias',v_payload->>'alias_type',coalesce((v_payload->>'verified')::boolean,false));
    when 'set_alias_verified' then v_id:=bullmatch.admin_set_alias_verified(v_payload->>'entity_type',(v_payload->>'alias_id')::uuid,(v_payload->>'verified')::boolean);
    when 'create_event' then v_id:=bullmatch.admin_create_event(v_payload);
    when 'update_event' then v_id:=bullmatch.admin_update_event((v_payload->>'id')::uuid,coalesce(v_payload->'patch','{}'::jsonb));
    when 'archive_event' then v_id:=bullmatch.admin_archive_event((v_payload->>'id')::uuid,v_payload->>'reason');
    when 'set_event_verification' then v_id:=bullmatch.admin_set_event_verification((v_payload->>'id')::uuid,v_payload->>'status');
    when 'create_match' then v_id:=bullmatch.admin_create_match(v_payload);
    when 'update_match' then v_id:=bullmatch.admin_update_match((v_payload->>'id')::uuid,coalesce(v_payload->'patch','{}'::jsonb));
    when 'archive_match' then v_id:=bullmatch.admin_archive_match((v_payload->>'id')::uuid,v_payload->>'reason');
    when 'add_match_participant' then v_id:=bullmatch.admin_add_match_participant((v_payload->>'match_id')::uuid,coalesce(v_payload->'data','{}'::jsonb));
    when 'update_match_participant' then v_id:=bullmatch.admin_update_match_participant((v_payload->>'id')::uuid,coalesce(v_payload->'patch','{}'::jsonb));
    when 'remove_match_participant' then v_id:=bullmatch.admin_remove_match_participant((v_payload->>'id')::uuid);
    when 'set_match_result' then v_id:=bullmatch.admin_set_match_result((v_payload->>'match_id')::uuid,v_payload->>'result_type',(v_payload->>'winner_participant_id')::uuid,v_payload->>'reason');
    when 'set_match_verification' then v_id:=bullmatch.admin_set_match_verification((v_payload->>'id')::uuid,v_payload->>'status');
    when 'set_match_published' then v_id:=bullmatch.admin_set_match_published((v_payload->>'id')::uuid,(v_payload->>'publish')::boolean);
    else raise exception 'unsupported admin operation: %',p_operation using errcode='22023';
  end case;
  return jsonb_build_object('ok',true,'operation',v_op,'id',v_id);
end;
$$;

revoke all on function public.bullmatch_api_public_query(text,uuid,text,integer,integer) from public,anon,authenticated;
revoke all on function public.bullmatch_api_member(uuid) from public,anon,authenticated;
revoke all on function public.bullmatch_api_admin_command(uuid,text,jsonb) from public,anon,authenticated;
grant execute on function public.bullmatch_api_public_query(text,uuid,text,integer,integer) to service_role;
grant execute on function public.bullmatch_api_member(uuid) to service_role;
grant execute on function public.bullmatch_api_admin_command(uuid,text,jsonb) to service_role;
