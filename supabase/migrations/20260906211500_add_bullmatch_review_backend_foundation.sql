...
-- Align the implemented claim state machine with the approved Contribution & Trust contract.
...
-- Explicit linkage keeps review-case composition auditable instead of hiding claim IDs in JSON only.
...
create index if not exists review_case_claims_claim_idx
  on bullmatch_private.review_case_claims(claim_id, review_case_id);
...
-- Internal authorization helper. Browser roles cannot execute it.
...
begin
  if p_actor_id is null then
    raise exception 'review actor is required' using errcode='22023';
  end if;
  ...
  where au.user_id = p_actor_id
    ...
    and au.role in ('ADMIN','REVIEWER');

  if v_role is null then
    raise exception 'ACTIVE ADMIN or REVIEWER required' using errcode='42501';
  end if;
...
-- Controlled queue/detail read bridge. Evidence remains behind the service-only boundary.
  ...
  case v_resource
      ...
      from (
        select
          ...
          (select count(*)
            ...
            where rcc.review_case_id=rc.id) as evidence_link_count,
          (rc.case_type in ('CONFLICTING_RESULT','CONFLICTING_DATE','MERGE_SPLIT')
            or coalesce((rc.context->>'has_conflict')::boolean,false)) as has_conflict,
    ...
    when 'REVIEW_CASE' then
      if p_id is null then
        raise exception 'p_id is required for REVIEW_CASE' using errcode='22023';
      end if;
      ...
      select jsonb_build_object(
        'case', jsonb_build_object(
          ...
          'subject_type',rc.subject_type,
          'subject_ref',rc.subject_ref,
          'summary',rc.summary,
          'context',rc.context,
          'assigned_to',rc.assigned_to,
          'assigned_display_name',au.display_name,
          'case_version',rc.case_version,
        ...
        'evidence', coalesce((
          select jsonb_agg(jsonb_build_object(
            ...
            'timestamp_start_seconds',e.timestamp_start_seconds,
            'timestamp_end_seconds',e.timestamp_end_seconds,
            'content_sha256',e.content_sha256,
            'source',jsonb_build_object(
              'id',si.id,
              'title',case when e.access_class='RESTRICTED' then null else si.title end,
              'canonical_url',case when e.access_class='RESTRICTED' then null else si.canonical_url end,
              'published_at',si.published_at,
              'retrieved_at',si.retrieved_at,
              'source_name',s.name,
              'reliability_tier',s.reliability_tier
            )
          ) order by
            case ce.relationship when 'CONTRADICTS' then 1 when 'SUPPORTS' then 2 else 3 end,
            e.created_at,
            e.id)
          from bullmatch_private.review_case_claims rcc
          join bullmatch_private.claim_evidence ce on ce.claim_id=rcc.claim_id
        ...
        'entity_match_candidates',coalesce((
          ...
          where emc.review_case_id=rc.id
        ),'[]'::jsonb),
        'duplicate_candidates',coalesce((
          ...
          where dc.review_case_id=rc.id
        ),'[]'::jsonb),
        'history',coalesce((
          select jsonb_agg(jsonb_build_object(
            'id',ra.id,
            'actor_id',ra.actor_id,
            'actor_display_name',hist_user.display_name,
            'action',ra.action,
            'command_id',ra.command_id,
            'expected_case_version',ra.expected_case_version,
            'case_version_before',ra.case_version_before,
            'case_version_after',ra.case_version_after,
            'before_value',ra.before_value,
            'after_value',ra.after_value,
            'notes',ra.notes,
            'created_at',ra.created_at
          ) order by ra.created_at,ra.id)
          from bullmatch.review_actions ra
          left join bullmatch.app_users hist_user on hist_user.user_id=ra.actor_id
...
-- Idempotent + optimistic review command bridge.
-- This function changes review/claim state only. It never mutates canonical Bulls/Matches.
create or replace function public.bullmatch_api_review_command(
  p_actor_id uuid,
  p_command jsonb
)
returns jsonb
...
begin
  ...
  if coalesce(v_command->>'schema_version','') <> '1.0.0' then
    raise exception 'unsupported review command schema_version' using errcode='22023';
  end if;
  ...
  if v_expected_version < 1 then
    raise exception 'expected_case_version must be >= 1' using errcode='22023';
  end if;
  if v_expected_status not in ('OPEN','IN_REVIEW','RESOLVED','REJECTED','CANCELLED') then
    raise exception 'invalid expected_case_status' using errcode='22023';
  end if;
  ...
  if found then
    ...
    return jsonb_build_object(
      ...
      'case_version',v_existing.case_version_after
    );
  end if;
  ...
  -- Re-check after row lock so concurrent duplicate command submissions replay safely.
  select * into v_existing from bullmatch.review_actions where command_id=v_command_id;
  if found then
    if v_existing.review_case_id <> v_case_id or v_existing.action <> v_action then
      raise exception 'command_id already belongs to a different review command' using errcode='23505';
  ...
  if v_case.case_version <> v_expected_version or v_case.status <> v_expected_status then
    raise exception 'STALE_REVIEW_CASE expected %/% but current is %/%',
      v_expected_status,v_expected_version,v_case.status,v_case.case_version
      using errcode='40001';
  end if;
  ...
  if v_action = 'CLAIM' then
    if v_case.status not in ('OPEN','IN_REVIEW') then
      raise exception 'case cannot be claimed from status %',v_case.status using errcode='22023';
    end if;
    if v_case.assigned_to is not null and v_case.assigned_to <> p_actor_id then
      raise exception 'review case is already assigned' using errcode='40001';
    end if;
    update bullmatch.review_cases
      set assigned_to=p_actor_id,status='IN_REVIEW',case_version=case_version+1,updated_at=now()
      where id=v_case_id;

  elsif v_action = 'UNCLAIM' then
    if v_case.status <> 'IN_REVIEW' then
      raise exception 'only IN_REVIEW cases can be unclaimed' using errcode='22023';
    end if;
    if v_case.assigned_to <> p_actor_id and v_role <> 'ADMIN' then
      raise exception 'only the assigned reviewer or ADMIN can unclaim this case' using errcode='42501';
    end if;
    update bullmatch.review_cases
      set assigned_to=null,status='OPEN',case_version=case_version+1,updated_at=now()
      where id=v_case_id;
    ...
    if v_case.status not in ('OPEN','IN_REVIEW') then
      raise exception 'terminal claim decisions require OPEN or IN_REVIEW case' using errcode='22023';
    end if;

    if jsonb_typeof(v_payload->'claim_ids')='array' then
      select coalesce(array_agg(x::uuid),array[]::uuid[]) into v_claim_ids
      from jsonb_array_elements_text(v_payload->'claim_ids') as t(x);
    end if;
    if cardinality(v_claim_ids)=0 then
      select coalesce(array_agg(rcc.claim_id order by rcc.claim_id),array[]::uuid[])
        into v_claim_ids
      from bullmatch_private.review_case_claims rcc
      where rcc.review_case_id=v_case_id;
    end if;
    if cardinality(v_claim_ids)=0 then
      raise exception 'review case has no linked claim_ids to decide' using errcode='22023';
    end if;
    if exists (
      select 1 from unnest(v_claim_ids) x
      where not exists (
        select 1 from bullmatch_private.review_case_claims rcc
        where rcc.review_case_id=v_case_id and rcc.claim_id=x
      )
    ) then
    ...
    if v_action='APPROVE' then
      update bullmatch_private.claims
         set status='VERIFIED'
       where id=any(v_claim_ids)
         and status in ('PROPOSED','REVIEW_REQUIRED','CORROBORATED');
      get diagnostics v_updated = row_count;
      if v_updated <> cardinality(v_claim_ids) then
        raise exception 'APPROVE requires non-conflicted, non-terminal linked claims' using errcode='22023';
      end if;
      update bullmatch.review_cases
      ...
      update bullmatch_private.claims
         set status='REJECTED'
       where id=any(v_claim_ids)
         and status not in ('VERIFIED','SUPERSEDED','WITHDRAWN');
      get diagnostics v_updated = row_count;
      if v_updated <> cardinality(v_claim_ids) then
        raise exception 'REJECT cannot overwrite VERIFIED/SUPERSEDED/WITHDRAWN claim state' using errcode='22023';
      end if;
      update bullmatch.review_cases
        set status='REJECTED',resolved_at=now(),resolved_by=p_actor_id,
            case_version=case_version+1,updated_at=now()
        where id=v_case_id;

    else
      v_resolution := upper(btrim(coalesce(v_payload->>'resolution','')));
      if v_resolution not in ('VERIFIED','REJECTED','SUPERSEDED','CONFLICT') then
        raise exception 'RESOLVE_CONFLICT resolution must be VERIFIED, REJECTED, SUPERSEDED, or CONFLICT' using errcode='22023';
      end if;
      update bullmatch_private.claims
      ...
      get diagnostics v_updated = row_count;
      if v_updated <> cardinality(v_claim_ids) then
        raise exception 'cannot resolve WITHDRAWN claim' using errcode='22023';
      end if;
      update bullmatch.review_cases
    ...
    if v_case.status not in ('RESOLVED','REJECTED','CANCELLED') then
      raise exception 'only terminal review cases can be reopened' using errcode='22023';
    end if;
    if v_note is null then raise exception 'REOPEN requires note' using errcode='22023'; end if;
    update bullmatch.review_cases
      set status='OPEN',assigned_to=null,resolved_at=null,resolved_by=null,
          case_version=case_version+1,updated_at=now()
      where id=v_case_id;
  ...
  insert into bullmatch.review_actions(
    ...
    v_expected_version,v_case.case_version,v_before,v_after,v_note
  ...
  insert into bullmatch_private.audit_log(
    ...
    'USER',p_actor_id::text,'REVIEW_'||v_action,'REVIEW_CASE',v_case_id,v_command_id,
    jsonb_build_object(
      'review_action_id',v_review_action_id,
      'case_version_before',v_expected_version,
      'case_version_after',v_case.case_version,
      'claim_ids',to_jsonb(v_claim_ids),
